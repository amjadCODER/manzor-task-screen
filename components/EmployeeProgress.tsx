  ...
  return (
    <section className="glass-panel flex min-h-0 flex-col rounded-[32px] p-5">
      <div className="mb-4 shrink-0">
        ...
        <p className="mt-1 text-sm text-slate-300">اضغط على الموظف لعرض مهامه كاملة</p>
      </div>
      <div className="no-scrollbar grid min-h-0 flex-1 grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-4 overflow-y-auto pr-1">
        {employees.map((employee) => (
          <button type="button" onClick={() => onSelect(employee)} key={employee.id} className="rounded-3xl border border-white/10 bg-white/[0.07] p-4 text-center transition hover:-translate-y-1 hover:border-cyan-300/40 hover:bg-white/[0.11]">
            <Donut value={getCompletionRate(employee)} label={employee.name} size="sm" />
            <p className="mt-2 text-xs text-slate-300">{employee.completedToday} من {employee.totalToday} مهام</p>
            <p className="mt-2 text-[11px] font-bold text-cyan-200">عرض التفاصيل</p>
          </button>
        ))}
      </div>
    </section>
