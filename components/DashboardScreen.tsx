...
export function DashboardScreen({ initialData }: { initialData: DashboardData }) {
  ...
  return (
    <main className="soft-grid h-screen w-screen overflow-hidden p-2 text-white sm:p-4 xl:p-6" dir="rtl">
      <section className="relative flex h-full w-full flex-col gap-3 overflow-hidden rounded-[28px] border border-white/10 bg-[#040816]/75 p-3 shadow-[0_30px_120px_rgba(0,0,0,0.45)] sm:rounded-[42px] sm:p-4 xl:gap-5 xl:p-5">
        ...
        <div className="absolute left-4 top-4 z-40 flex gap-2">
          ...
          <button onClick={() => router.push("/")} title="دخول الموظف" className="rounded-2xl border border-white/15 bg-black/35 px-3 py-2 text-sm font-black text-white backdrop-blur-xl">دخول</button>
        </div>

        <section className="relative z-10 grid min-h-0 flex-1 grid-cols-12 gap-3 xl:gap-5">
          <div className="col-span-5 min-h-0"><CurrentTasks tasks={data.currentTasks} /></div>
          <div className="col-span-4 min-h-0"><EmployeeProgress employees={employees} onSelect={setSelected} /></div>
          <div className="col-span-3 flex min-h-0 flex-col gap-3 xl:gap-5"><TopPerformer employee={topEmployee} /><CompletedTasks tasks={data.completedTasks} /></div>
        </section>
        <FooterStats {...totals} />
